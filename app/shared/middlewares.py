...
class AuthorizationMiddleware(MiddlewareProtocol):
    ...
    def validate_user(self, user_id: str):
        ...
        user_details_cursor = user_instance.find_one(
            {'_id': ObjectId(user_id)}, {'_id': True}
        )
    ...
    def validate_auth_token(token: str):
        try:
            token_str = get_bearer_string(token)
            ...
            if token_time > current_timestamp():
                ...
                raise NotAuthorizedException(detail=ErrorMessages.INVALID_TOKEN.value)
